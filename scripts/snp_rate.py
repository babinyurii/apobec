...
def count_snp_per_read(ref_seq, in_file):
    ...
    - each int value is a num of snp in a read
    """
    snpes_per_read = []
...
def create_strip_plot(df, time_stamp):
    ...
    plt.ylabel("number of SNP", fontsize=15)
    plt.xlabel("input file name", fontsize=15)
    plt.tight_layout()
    fig.savefig("./output_apobec/mutation_rate_distribution_" + time_stamp +  "_.png") 
...
def get_mode_median(df_raw_count):
    """calcualte mode and median
    using raw snp count
    """
    mode = df_raw_count.mode()
    mode.index = ["mode"] # put in list, otherwise an error "a collection of some kind"
...
def calculate_non_mutated(pivot_df, df_raw_count):
    ...
    df_non_mutated = pd.DataFrame([non_mutated], columns=cols) # list is in [] as nested, to get values in a row in df
    df_non_mutated.rename(index={0: "number of non-mutated reads"}, inplace=True)
...
def save_df(pivot_df, df_raw_count, time_stamp):
    ...
    writer = pd.ExcelWriter("./output_apobec/mutation_rate_" + time_stamp +'_.xlsx')
...
def main():
    ...
    if os.path.exists("./input_data"):
        ...
        df_raw_count.drop([0], inplace=True) # drop first raw which is a reference to itself comparison
